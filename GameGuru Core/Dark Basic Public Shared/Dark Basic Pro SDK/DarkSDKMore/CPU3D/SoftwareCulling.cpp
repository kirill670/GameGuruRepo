...
#define SHOW_LESS_OCCLUDEES_ON_HIGH_PRIM_CALLS
 ...
 BVH* g_pOccluderBVH = NULL;
 BVH* g_pOccludeeBVH = NULL;
...
//============================================================================
 ...
 void OccluderConstructor ( void )
 {
	...
	g_pOccluderBVH = new BVH(OccluderList);
	g_pOccludeeBVH = new BVH(OccludeeList);
 ...
 void OccluderDestructor ( void )
 {
	...
	if (g_pOccluderBVH)
	{
		delete g_pOccluderBVH;
		g_pOccluderBVH = NULL;
	}
	if (g_pOccludeeBVH)
	{
		delete g_pOccludeeBVH;
		g_pOccludeeBVH = NULL;
	}
 }
...
{
	// build BVH trees
	g_pOccluderBVH->build();
	g_pOccludeeBVH->build();
	...
	int objectID;
	if (g_pOccluderBVH->traverse(cam.pos, cam.dir, objectID))
	{
		if (DrawOccluder(objectID))
		{
			OccluderListDrawn.push_back(objectID);
	...
	int objectID;
	if (g_pOccludeeBVH->traverse(cam.pos, cam.dir, objectID))
	{
		sObject* p = GetObjectData(objectID);
		if (p)
		{
			p->bUniverseVisible = true;
		}
...
[end of GameGuru Core/Dark Basic Public Shared/Dark Basic Pro SDK/DarkSDKMore/CPU3D/SoftwareCulling.cpp]
